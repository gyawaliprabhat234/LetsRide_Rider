...
namespace Uber_Rider.Helpers
{
    public class MapFunctionHelper
    {
        ...
        public double EstimateFares(VehicleType vehicle)
        {
            double basefare = vehicle.BaseFare; //NPR 
            double distanceFare = vehicle.KmFare; //NPR per kilometer
            double timefare = 1; //USD per minute

            if (distance / 1000 > vehicle.BaseKm)
            {
                double kmfares = ((distance / 1000) - vehicle.BaseKm) * distanceFare;
                double minsfares = (duration / 60) * timefare;

                double amount = kmfares  + basefare;
